...
# Debian 13 (trixie) - Riven + Zilean + Host Plex (VFS model, no rclone) - v23
#
# Installs/Configures:
...
deploy_zilean() {
  ...
  # Persist Zilean DB password so re-runs don’t desync Postgres credentials
  # (Postgres keeps the original password in its data volume; changing env later breaks auth).
  local zilean_pg_pw
  if [[ -f /opt/zilean/.env ]]; then
    # shellcheck disable=SC1091
    source /opt/zilean/.env
    zilean_pg_pw="${ZILEAN_POSTGRES_PASSWORD:-}"
  fi
  if [[ -z "${zilean_pg_pw:-}" ]]; then
    zilean_pg_pw="$(rand_hex_32)"
    cat >/opt/zilean/.env <<EOF
ZILEAN_POSTGRES_PASSWORD=${zilean_pg_pw}
EOF
