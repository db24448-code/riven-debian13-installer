...
# Debian 13 (trixie) - Riven + Zilean + Zurg + Host Plex (VFS model, no rclone) - v19
#
# Installs/Configures:
...
#
# Architecture:
# RealDebrid -> Zurg -> Riven -> RivenVFS mount -> Host Plex
#
...
############################
deploy_zurg_config() {
  log "Creating /opt/zurg/config/config.yml (required)"
  ...
  if [[ ! -f /opt/zurg/config/config.yml ]]; then
    local rd_token
    rd_token="$(prompt "Real-Debrid API token for Zurg (https://real-debrid.com/apitoken)" "")"
    if [[ -z "$rd_token" ]]; then
      warn "No token provided. Writing placeholder; Zurg will not function until you edit it."
...
############################
# Riven stack (includes Zurg service)
############################
deploy_riven() {
  log "Deploying Riven backend + frontend + Postgres + Zurg (VFS model)"
...
post_checks() {
  ...
  curl -fsS --max-time 2 http://127.0.0.1:9999 >/dev/null 2>&1 && echo "OK: Zurg :9999" || warn "Zurg not reachable yet (docker logs zurg)"
  ...
  echo "  Zurg WebDAV:     http://zurg:9999/dav"
  echo
