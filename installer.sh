...
#     - ensures /opt/media/riven/mount exists
#     - bind-mounts the dir onto itself if not already a mountpoint
#     - makes it rshared
#
# Modes:
...
main(){
  ...
  if [[ -z "$mode" ]]; then
    echo "1) Install  2) Reconfigure (preserve DB+config)  3) Update  4) Test-only  5) Reset  6) Wipe DB + Reset"
