...
#
# ALPHA 1 BASELINE
# From now on, changes should be surgical and double-checked to avoid removing/disabling large blocks.
#
# Characteristics:
...
#   ./installer.sh --update         -> pull images + restart in safe order + optional tests
...
#   ./installer.sh --wipe-server    -> DANGER: remove stack + UNINSTALL DOCKER + wipe Docker data (for fresh-server testing)
...
# 1) sudo caller
# 2) logname user (if non-root)
# 3) first /home user with uid>=1000
# 4) fallback 1000:1000
detect_host_uid_gid(){
  local uid="" gid="" u=""
...
# Inline reset logic (used if media-reset.sh is missing)
inline_reset(){
...
# Try to recover if the mount got stuck
fuse_umount /opt/media/riven/mount
umount -l /opt/media/riven/mount 2>/dev/null || true

# Re-assert rshared on host mountpoint
mkdir -p /opt/media/riven/mount
if ! findmnt -n /opt/media/riven/mount >/dev/null 2>&1; then
  mount --bind /opt/media/riven/mount /opt/media/riven/mount 2>/dev/null || true
fi
...
run_tests(){
  ...
  # Host-side checks are the reliable truth
...
do_update(){
  ...
  # Re-assert DB perms (useful after restores/recreates)
...
write_compose_files(){
  ...
  # Plex: host networking (LAN reliable)
...
services:
  ...
  zilean:
    ...
    # Relaxed healthcheck: if curl/wget missing, don't block startup; rely on host-side tests.
    healthcheck:
...
services:
  ...
  riven:
    ...
    environment:
      ...
      # Plex (Plex in host network mode -> use HOST_IP)
...
do_reconfigure(){
  ...
  local cur_host_ip cur_rd cur_api cur_auth
  cur_host_ip="$(env_get "${RIVEN_ENV}" HOST_IP || echo "$(detect_host_ip)")"
  cur_rd="$(env_get "${RIVEN_ENV}" REALDEBRID_API_KEY || true)"
  cur_api="$(env_get "${RIVEN_ENV}" API_KEY || true)"
  cur_auth="$(env_get "${RIVEN_ENV}" AUTH_SECRET || true)"
  ...
  if [[ "$(read_yesno "Rotate API_KEY + AUTH_SECRET? (breaks existing UI sessions)" "n")" == "y" ]]; then
    ...
    [[ -n "${cur_api}" ]] || env_set "${RIVEN_ENV}" "API_KEY" "$(gen_hex32)"
    [[ -n "${cur_auth}" ]] || env_set "${RIVEN_ENV}" "AUTH_SECRET" "$(gen_b64)"
  fi
...
wipe_server(){
  ...
  log "If you purged prereqs, you may need to reinstall ca-certificates/curl/gnupg before the next install."
}
...
main(){
  ...
  if [[ -z "$mode" ]]; then
    echo "1) Install  2) Reconfigure  3) Update  4) Test-only  5) Reset  6) Wipe DB + Reset  7) WIPE-SERVER (uninstall Docker too)"
  ...
  case "$mode" in
    --install)
      ...
      local host_ip
      host_ip="$(env_get "${RIVEN_ENV}" HOST_IP || echo "$(detect_host_ip)")"
      [[ "$(read_yesno "Run post-install tests now?" "y")" == "y" ]] && run_tests "$host_ip" || true
      ;;
