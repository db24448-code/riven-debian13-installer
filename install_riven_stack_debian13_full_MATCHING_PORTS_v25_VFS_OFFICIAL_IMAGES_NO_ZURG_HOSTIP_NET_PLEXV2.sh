...
# v25: Debian 13 (Trixie) (Plex repo/key v2 fix) - Riven VFS model (NO ZURG) + Zilean + host Plex
...
install_plex_apt() {
  ...
  # Debian 13 (Trixie) uses sqv/Sequoia for signature verification and rejects SHA1-bound keys.
  # Plex provides a v2 signing key and new repo endpoint to satisfy modern verification policy.
  rm -f /usr/share/keyrings/plex.gpg /usr/share/keyrings/plexmediaserver.v2.gpg || true
  curl -fsSL https://downloads.plex.tv/plex-keys/PlexSign.v2.key | gpg --dearmor -o /usr/share/keyrings/plexmediaserver.v2.gpg
  chmod 644 /usr/share/keyrings/plexmediaserver.v2.gpg

  echo "deb [signed-by=/usr/share/keyrings/plexmediaserver.v2.gpg] https://repo.plex.tv/deb public main" >/etc/apt/sources.list.d/plexmediaserver.list
  apt-get update -y
