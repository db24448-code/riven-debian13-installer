...
# Plex migrated repositories starting with PMS v1.43.0; Debian 13 enforces stronger signature policy (SHA1 rejected).
# Use Plex's current repo + v2 signing key (per Plex Support).
# Remove any legacy Plex repo entries that can break apt update on Debian 13
rm -fv /etc/apt/sources.list.d/plex* /etc/apt/sources.list.d/plexmediaserver* || true
rm -fv /usr/share/keyrings/plex.gpg || true
mkdir -p /etc/apt/keyrings
curl -fsSL https://downloads.plex.tv/plex-keys/PlexSign.v2.key | gpg --dearmor -o /etc/apt/keyrings/plexmediaserver.v2.gpg
chmod a+r /etc/apt/keyrings/plexmediaserver.v2.gpg
