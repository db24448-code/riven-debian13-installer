...
post_checks() {
  ...
  # Zurg
  if retry 20 2 -- curl -fsS http://127.0.0.1:9999/ >/dev/null 2>&1; then
    echo "OK: Zurg reachable"
  else
    warn "Zurg not reachable yet (docker logs zurg). If token is placeholder, Zurg may still start but functionality will be limited."
  fi
