...
# Debian 13 - Full Riven/Zilean Stack Reset (hardened v9, VFS model, no rclone)
...
if command -v docker >/dev/null 2>&1; then
  ...
  if docker info >/dev/null 2>&1; then
    ...
    if [[ -f /opt/zilean/docker-compose.yml ]]; then
      ...
      # Extra safety: remove any leftover Zilean volumes (compose project name can vary)
      docker volume ls -q | grep -iE "zilean.*pg|pg.*zilean|zilean-pg" | xargs -r docker volume rm -f 2>/dev/null || true
