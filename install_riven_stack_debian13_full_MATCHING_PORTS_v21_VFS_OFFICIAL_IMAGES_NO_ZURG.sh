...
# Debian 13 (trixie) - Riven + Zilean + Host Plex (VFS model, no rclone) - v21
#
# Installs/Configures:
...
deploy_riven() {
  ...
  plex_url="$(prompt "Plex URL (Riven container -> host Plex). Recommend using host.docker.internal" "http://host.docker.internal:32400")"
...
EOF
  ...
  # - Updaters: Plex enabled if PLEX_TOKEN provided, using PLEX_URL (defaults to host.docker.internal)
