...
# Add your Real-Debrid API token (https://real-debrid.com/apitoken) to enable functionality.
zurg: v1
...
post_checks() {
  ...
  # Zurg
  if retry 20 2 -- bash -lc '</dev/tcp/127.0.0.1/9999' >/dev/null 2>&1; then
    echo "OK: Zurg port 9999 is open"
  else
    warn "Zurg port 9999 not open yet (docker logs zurg)."
  fi
  # Zurg token sanity (warn if Real-Debrid rejects token)
  if docker logs --tail=200 zurg 2>/dev/null | grep -qi 'bad_token'; then
    warn "Zurg is running but Real-Debrid token was rejected (bad_token/401). Update /opt/zurg/config/config.yml and restart zurg."
  fi
  ...
  if retry 30 2 -- curl -fsS http://127.0.0.1:8181/healthchecks/ping >/dev/null 2>&1; then
    ...
    docker ps --format "table {{.Names}}\t{{.Status}}" | sed -n "1,12p" || true
    (cd /opt/zilean && docker compose logs --tail=120 zilean) || true
  fi

  # Riven
  if retry 40 2 -- curl -fsS http://127.0.0.1:8080/docs >/dev/null 2>&1; then
